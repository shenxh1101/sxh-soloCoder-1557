      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="card animate-fadeInUp">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (material) {
                    setUnitPrice(material.currentPrice.toString());
                  }
                }}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-brown-500 mb-2">
                  采购数量 ({selectedMaterialData?.unit || '单位'})
                </label>
                <input
                  type="number"
                  step="0.01"
              ...
              <div>
                <label className="block text-sm font-medium text-brown-500 mb-2">
                  单价 (元/{selectedMaterialData?.unit || '单位'})
                </label>
                <input
                  type="number"
                  step="0.01"
        ...
        <div className="card animate-fadeInUp delay-100" style={{ animationDelay: '0.1s' }}>
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-bold text-brown-500">
              {selectedDate} 采购记录
            </h3>
            ...
            <div className="space-y-3 max-h-96 overflow-y-auto pr-2">
              {todayPurchases.map(purchase => {
                ...
                return (
                  <div
                    key={purchase.id}
                    className="flex items-center justify-between p-4 bg-warm-50 rounded-xl hover:bg-warm-100 transition-colors group"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-brown-400">
                          {purchase.quantity} {material?.unit} × ¥{purchase.unitPrice}/{material?.unit}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      ...
                      <button
                        onClick={() => {
                          if (confirm('确定要删除这条采购记录吗？库存将同步更新。')) {
                    ...
                    <tr key={purchase.id} className="border-b border-warm-100 hover:bg-warm-50">
                      ...
                      <td className="py-3 px-4">
                        <span className="flex items-center gap-2">
                          ...
                          <span className="text-brown-500">{material?.name}</span>
                        </span>
                      </td>
